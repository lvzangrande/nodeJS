...
export const produtoRoutes = express.Router();//criar o roteador

produtoRoutes.get('/',async(req,resolve, next)=>{
    try{
        const produtos = await catalogo.listar();
        resolve.status(200).json({sucesso:true, dados:produtos})
    }catch(erro){
        next(erro);//encaminha para middleware global de erro(provisório)
    }
});
